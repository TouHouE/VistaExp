...
def iter_slice_patch(
    ...
    for adpt_pseudo_bs, slice_idx in zip(map(len, seq_slice_ids), seq_slice_ids):
        ...
        if adpt_pseudo_bs > 1:
            ...
            pred_mask = outputs[0]['low_res_logits'].permute(1, 0, 3, 4).contiguous()
        pred_mask = torch.stack(post_pred(decollate_batch(pred_mask)), dim=1)
        pred_record.append(pred_mask.cpu().item())
        target_record.append(target.cpu().item())

        del inputs, labels, pred_mask, target
        gc.collect()
        torch.cuda.empty_cache()
    quasi3d_pred = torch.cat(pred_record, dim=0)[None].permute(0, 2, 1, 3, 4)  # (B=1) x N_class x N_patch x H x W
