...
from engine.poor_vram_engine import do_validation
...
def run_training(
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if (epoch + 1) % args.val_every == 0:
            ...
            epoch_time = time.time()
            meter = do_validation.val_epoch(
                model, val_loader,
                iterative=False, epoch=epoch, acc_func=acc_func,
                args=args, post_label=post_label, post_pred=post_pred, run=run
            )

            val_avg_acc = np.mean(meter.avg)
            val_MA = val_avg_acc if val_MA is None else .9 * val_MA + .1 * val_avg_acc

            if args.rank == 0:
                Terminate.show_valided_info(
                    epoch, val_avg_acc, val_MA, best_epoch, val_acc_max, epoch_time, meter=meter, args=args
                )
                if writer is not None:
                    writer.add_scalar("val_acc", val_avg_acc, epoch)
                    for class_idx in meter.class_dice.keys():
                        writer.add_scalar(f"class-{class_idx} Dice", meter.get_dice(class_idx), epoch)

                if run is not None:
                    log_dict = {
                        f'class-{class_idx} Dice': meter.get_dice(class_idx) for class_idx in meter.class_dice.keys()
                    }
                    log_dict.update({'val_acc': val_avg_acc, 'epoch': epoch})
                    run.log(log_dict)
