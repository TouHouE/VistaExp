...
from engine.normal_engine import phase1_train as PT1
from engine.normal_engine import phase2_train as PT2
...
def run_training(
        ...
        optimizer, loss_func, acc_func,
        args,
        scheduler=None, start_epoch=0,
        post_label=None, post_pred=None,
):
    writer, run, scaler = None, None, None
    ...
    val_acc_max, best_epoch, val_MA, best_log = 0.0, -1, None, dict()
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if epoch > args.iterative_training_warm_up_epoch:
            train_function = PT2.train_epoch(
                model, train_loader, optimizer, scaler=scaler, loss_func=loss_func,
                epoch=epoch, run=run, args=args
            )
        ...
        if (epoch + 1) % args.val_every == 0:
            ...
            val_avg_acc = val_epoch(
                model, val_loader,
                iterative=False, epoch=epoch, acc_func=acc_func, args=args,
